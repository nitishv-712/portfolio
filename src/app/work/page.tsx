...
const EXPERIENCE = [
  {
    period: "Jan 2026 — Present", role: "Full-Stack Developer",
    company: "99HomeBazaar", location: "Noida, U.P", color: "#7c3aed",
    points: [
  ...
  {
    period: "Sept 2024 — Sept 2025", role: "Application Developer",
    company: "SampurnaKart Innovations Pvt Ltd", location: "Jalandhar, Punjab", color: "#2563eb",
    points: [
  ...
  {
    period: "June 2024 — Aug 2024", role: "Flutter Developer Intern",
    company: "SampurnaKart Innovations Pvt Ltd", location: "Jalandhar, Punjab", color: "#06b6d4",
    points: [
    ...
    <main style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <section className="hero-pad" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <h1 className="fade-up" style={{ fontSize: "clamp(32px, 5vw, 64px)", fontWeight: 800, letterSpacing: "-1px", marginBottom: 16, animationDelay: "0.15s", opacity: 0 }}>
            My <span className="gradient-text">Work</span>
          </h1>
          <p className="fade-up" style={{ color: "var(--text-muted)", fontSize: "clamp(15px,2vw,18px)", maxWidth: 520, lineHeight: 1.75, animationDelay: "0.25s", opacity: 0 }}>
      ...
      {/* Projects */}
      <section className="section-pad" style={{ paddingTop: "40px !important", position: "relative", overflow: "hidden" }}>
        <div className="orb" style={{ width: 400, height: 400, background: "rgba(236,72,153,0.05)", bottom: "-80px", left: "-80px" }} />
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(300px,100%), 1fr))", gap: 16 }}>
            {PROJECTS.map((p, i) => (
              <div key={p.title} className="card reveal" style={{ padding: "24px 20px", display: "flex", flexDirection: "column", transitionDelay: `${i * 0.07}s` }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `${p.color}18`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                ...
                <p style={{ color: "var(--text-muted)", fontSize: 13, lineHeight: 1.7, marginBottom: 16, flex: 1 }}>{p.desc}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
                  {p.tech.map((t, j) => <span key={t} className={`tag ${TAG_COLORS[j % TAG_COLORS.length]}`}>{t}</span>)}
                </div>
                <a href={p.href} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 13, fontWeight: 600, color: p.color, textDecoration: "none" }}
      ...
      <section className="section-pad" style={{ background: "var(--bg-subtle)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <h2 className="reveal reveal-delay-1" style={{ fontSize: "clamp(24px, 3vw, 40px)", fontWeight: 800, marginBottom: 44 }}>
            Where I've <span className="gradient-text">worked</span>
          </h2>

          <div style={{ display: "flex", flexDirection: "column" }}>
            {EXPERIENCE.map((exp, i) => (
              <div key={i} className="reveal" style={{ display: "flex", gap: 20, paddingBottom: i < EXPERIENCE.length - 1 ? 40 : 0, transitionDelay: `${i * 0.1}s` }}>
                {/* Timeline */}
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 16, flexShrink: 0 }}>
                  <div style={{ width: 12, height: 12, borderRadius: "50%", background: exp.color, flexShrink: 0, marginTop: 4, boxShadow: `0 0 10px ${exp.color}88` }} />
                ...
                {/* Content */}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: exp.color, fontWeight: 700, letterSpacing: 0.5, marginBottom: 4 }}>{exp.period}</div>
                  <h3 style={{ fontSize: "clamp(15px,2vw,18px)", fontWeight: 700, marginBottom: 4 }}>{exp.role}</h3>
                  <div style={{ fontSize: 13, color: "var(--text-muted)", fontWeight: 600, marginBottom: 12 }}>
                    {exp.company} · {exp.location}
                  </div>
                  <ul style={{ paddingLeft: 16 }}>
                    {exp.points.map((pt, j) => (
                      <li key={j} style={{ color: "var(--text-muted)", fontSize: 13, lineHeight: 1.8, marginBottom: 4 }}>{pt}</li>
                    ))}
                  </ul>
                </div>
              </div>
